...
class BertForSequenceClassificationUserDefined(BertPreTrainedModel):
    ...
    def forward(self, input_ids=None, attention_mask=None, token_type_ids=None, position_ids=None,
                head_mask=None, inputs_embeds=None, e1_pos=None, e2_pos=None, w=None):
        ...
        e_pos_mean = e_pos_output.view(e_pos_output.shape[0],2,-1)
        e_pos_output = self.dropout(e_pos_output)
        #+ e_pos_mean[:,0,:] + e_pos_mean[:,1,:]
        hidden = self.classifier(e_pos_output) + e_pos_mean[:,0,:] + e_pos_mean[:,1,:]
        logits = self.classifier_2(hidden)

        outputs = (logits,self.output_emebedding)   # add hidden states and attention if they are here
